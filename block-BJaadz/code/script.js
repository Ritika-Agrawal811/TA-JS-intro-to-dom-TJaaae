...
allHrsArray.forEach((value) => {
  value.style.border = "1px solid tomato";
});
...
allHrsArray.forEach((value) => {
  if ([...value.classList].includes("image")) {
    value.style.border = "2px solid purple";
  }
});
...
allHrsArray.forEach((value) => {
  value.style.border = "1px dashed tomato";
});
...
box3.insertBefore(para3, box3.firstElementChild);
...
box6.style.color = "#000";

// Change the font size of the para inside box 1 to 0.8rem.
let paraBox1 = box1.querySelector("p");

paraBox1.style.fontSize = "0.8rem";
...
allBoxes.forEach((value, index) => {
  if (index % 2 == 0) {
    value.style.backgroundColor = "aliceblue";
  }
});
